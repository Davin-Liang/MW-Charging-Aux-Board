...
void MainWindow::on_pushButton_motor_control_clicked()
{
    ...
    QByteArray command = commandBuilder.buildMotorCommand(x, y, speed);
...
void MainWindow::on_pushButton_find_optimal_clicked()
{
    ...
    QByteArray command = commandBuilder.buildFindOptimalCommand(trajType, circleRadius,
...
void MainWindow::on_pushButton_read_motor_clicked()
{
    ...
    QByteArray command = commandBuilder.buildReadCommand(MOTOR_DATA_READ);
    tcpSocket->write(command);
...
void MainWindow::on_pushButton_read_optimal_clicked()
{
    ...
    QByteArray command = commandBuilder.buildReadCommand(CMD_OPT_RES_READ);
    tcpSocket->write(command);
...
void MainWindow::on_pushButton_read_channel_clicked()
{
    ...
    QByteArray command = commandBuilder.buildReadCommand(CURRENT_VPCH_READ);
    tcpSocket->write(command);
...
void MainWindow::on_pushButton_send_time_clicked()
{
    ...
    QByteArray command = commandBuilder.buildTimeCommand();
    tcpSocket->write(command);
...
void MainWindow::initializeControlCommands()
{
    ...
    // 设置默认值
    MotorCmd_t motorCmd = commandBuilder.getDefaultMotorCmd();
    FindOptimalCmd_t findOptCmd = commandBuilder.getDefaultFindOptCmd();

    ui->lineEdit_motor_x->setText(QString::number(motorCmd.x));
    ui->lineEdit_motor_y->setText(QString::number(motorCmd.y));
    ui->lineEdit_motor_speed->setText(QString::number(motorCmd.speed));
    ui->lineEdit_circle_radius->setText(QString::number(findOptCmd.cirTrajRad));
    ui->lineEdit_square_step->setText(QString::number(findOptCmd.squThajStepLen));
    ui->lineEdit_max_voltage->setText(QString::number(findOptCmd.maxVol));
    ui->lineEdit_voltage_step->setText(QString::number(findOptCmd.volStepLen));
...
void MainWindow::onSocketReadyRead()
{
    ...
    while (commandBuilder.parseCommandFrame(receiveBuffer, frame)) {
        // 处理完整的命令帧
