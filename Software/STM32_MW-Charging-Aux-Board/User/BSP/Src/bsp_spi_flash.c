...
void SPI_FLASH_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; // 软件控制，推挽输出
  ...
  SPI_InitStructure.SPI_DataSize = SPI_DataSize_8b; // 数据帧长度为 8 位
...
void SPI_FLASH_BufferWrite(u8* pBuffer, u32 WriteAddr, u16 NumByteToWrite)
{
  u8 NumOfPage = 0, NumOfSingle = 0, Addr = 0, count = 0, temp = 0;
	...
	/* mod运算求余，若 writeAddr 是 SPI_FLASH_PageSize 整数倍，运算结果 Addr 值为 0 */
...
u8 SPI_FLASH_ReadByte(void)
{
 ...
 /**
   ...
   * @param  byte：要发送的数据
   * @retval 返回接收到的数据
   * @note   函数中不包含 SPI 起始和停止信号，只是收发的主要过程，所以在调用本函数前后要做好起始和停止信号的操作
   **/
