...
void SD_LowLevel_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  /* 清除指定 DMA 通道（SD_SDIO_DMA_STREAM）的所有可能标志位 */
  DMA_ClearFlag(SD_SDIO_DMA_STREAM, SD_SDIO_DMA_FLAG_FEIF | SD_SDIO_DMA_FLAG_DMEIF | SD_SDIO_DMA_FLAG_TEIF | SD_SDIO_DMA_FLAG_HTIF | SD_SDIO_DMA_FLAG_TCIF);

  /* 禁止 DMA 流，然后恢复默认配置，保证初始化时是干净的。 */
  /* DMA2 Stream3  or Stream6 disable */
  DMA_Cmd(SD_SDIO_DMA_STREAM, DISABLE);
  /* DMA2 Stream3  or Stream6 Config */
  DMA_DeInit(SD_SDIO_DMA_STREAM);

  SDDMA_InitStructure.DMA_Channel = SD_SDIO_DMA_CHANNEL; // 指定 DMA 使用的通道号
  SDDMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)SDIO_FIFO_ADDRESS; // SDIO 的 FIFO 地址
  SDDMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)BufferSRC; // 数据源缓冲区
  SDDMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral; // 内存 → 外设
  SDDMA_InitStructure.DMA_BufferSize = 1; // 这里表面上是只配置传输 1 个 word，
                                          // 其实 SDIO 控制器自己会反复触发 DMA 请求，直到完成整个块传输，所以不用在这设置真实的缓冲区大小
  SDDMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable; // 外设地址固定（SDIO FIFO 地址不会变）
  SDDMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable; // 内存地址递增（逐个取数据）
  SDDMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word; // 数据宽度：Word（32位）
  SDDMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Word;
  SDDMA_InitStructure.DMA_Mode = DMA_Mode_Normal; // 普通模式
  SDDMA_InitStructure.DMA_Priority = DMA_Priority_VeryHigh; // 高优先级
  SDDMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Enable; // 使用 FIFO，提高效率
  SDDMA_InitStructure.DMA_FIFOThreshold = DMA_FIFOThreshold_Full; // FIFO 满阈值时传输
  SDDMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_INC4; // 内存端和外设端都采用突发 4-word 方式（加快传输效率）
  SDDMA_InitStructure.DMA_PeripheralBurst = DMA_PeripheralBurst_INC4;
  DMA_Init(SD_SDIO_DMA_STREAM, &SDDMA_InitStructure);
  DMA_ITConfig(SD_SDIO_DMA_STREAM, DMA_IT_TC, ENABLE); // 使能 传输完成中断 (TC)，传输结束会产生中断
  DMA_FlowControllerConfig(SD_SDIO_DMA_STREAM, DMA_FlowCtrl_Peripheral); // 流控器交给 外设控制，即由 SDIO 控制器决定 DMA 传输节奏  

  /* DMA2 Stream3  or Stream6 enable */
  DMA_Cmd(SD_SDIO_DMA_STREAM, ENABLE); // 最后开启 DMA 流，等待 SDIO 请求数据，就会自动把 BufferSRC 里的数据搬运过去
...
SD_Error SD_Init(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising; // 上升沿采样（符合 SD 卡协议）
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable; // 禁用旁路时钟，使用内部分频时钟
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable; // 不启用空闲时钟省电模式，初始化阶段通常关闭（禁用），保证时钟连续，方便调试和稳定性
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b; // 设置数据宽度为 1b
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable; // 禁用硬件流控，由软件控制数据传输时序
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
  {
    ...
    // 执行发送 CMD2，CMD2 是用于通知所有卡通过 CMD 线返回 CID 值
    SDIO_CmdInitStructure.SDIO_Argument = 0x0; // CMD2 的参数固定为 0
    SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_ALL_SEND_CID; // 设置命令索引为 CMD2
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Long; // CMD2 返回的数据是 长响应（136 位，R2 响应），因为 CID 是 128 位，需要长响应模式
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No; // 发送命令时不需要等待数据
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable; // 启动 CPSM（Command Path State Machine，命令路径状态机），让 SDIO 外设真正发出命令
    SDIO_SendCommand(&SDIO_CmdInitStructure); // 调用库函数发送命令，把上面配置的参数交给 SDIO 硬件去执行 

    errorstatus = CmdResp2Error(); // 使用 SDIO_GetResponse 函数获取响应内容
      ...
      ||  (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; // 响应类型是 R6 响应（短响应，48 位）
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&SDIO_CmdInitStructure);

    errorstatus = CmdResp6Error(SD_CMD_SET_REL_ADDR, &rca); // rca 会被写入，用来标识这张卡
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
  {
    ...
    // 发送 CMD9 给指定 RCA 的 SD 卡使其发送返回其 CSD 寄存器内容
  ...
  // 执行 SD_InitializeCards 函数无错误后 SD 卡就已经处于数据传输模式下的待机状态
}
...
SD_Error SD_EnableWideBusOperation(uint32_t WideMode)
{
  ...
  else if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    {
      errorstatus = SDEnWideBus(ENABLE); // 使能使用宽数据线
      ...
      if (SD_OK == errorstatus)
      {
        ...
        SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;
        SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;
        SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_4b; // 指定使用4条数据线
        SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
        SDIO_Init(&SDIO_InitStructure); // 重新调用 SDIO_Init
      }
    }
    else
    {
...
SD_Error SD_WriteBlock(uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize)
{
  ...
  /* 使能相关中断 */
  SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR | SDIO_IT_STBITERR, ENABLE);
  /* 配置使能 SDIO 数据向 SD 卡的数据传输的 DMA 请求 */
  SD_LowLevel_DMA_TxConfig((uint32_t *)writebuff, BlockSize);
  /* 使能SDIO 发送 DMA 请求 */
  SDIO_DMACmd(ENABLE);
#endif

  /* 对于高容量的 SD 卡要求块大小必须为 512 字节 */
  if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
    BlockSize = 512;
    WriteAddr /= 512;
...
SD_Error SD_Erase(uint64_t startaddr, uint64_t endaddr)
{
  ...
  if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_ERASE_GRP_START; // 设置擦除起始地址命令
    ...
    SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_ERASE_GRP_END; // 设置擦除终止地址命令
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; // 短响应
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&SDIO_CmdInitStructure);
